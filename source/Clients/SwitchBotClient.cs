...
)
{
    ...
    ) where TRequest : SwitchBotRequest<TResponse> where TResponse : SwitchBotResponse
    {
        ...
        {
            var httpResponseData = await httpResponseMessage.Content.ReadFromJsonAsync<TResponse>(cancellationToken);
